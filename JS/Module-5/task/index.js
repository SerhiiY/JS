...
const getUsersByEyeColor = (arr, color) => {
  const users = arr.filter(el => el.eyeColor === color);
  return users;
};
...
const getUsersByGender = (arr, gender) => {
  const users = arr.filter(el => el.gender === gender).map(el => el.name);
  return users;
};
...
const getInactiveUsers = arr => {
  const users = arr.filter(el => !el.isActive);
  return users;
};
...
const getUserByEmail = (arr, email) => {
  const users = arr.find(el => el.email === email);
  return users;
};
...
const getUsersWithAge = (arr, min, max) => {
  const users = arr.filter(el => max > el.age && el.age > min);
  return users;
};
...
const getTotalBalance = arr => {
  const balance = arr.reduce((acc, el) => acc + el.balance, 0);
  return balance;
};
...
const getUsersByFriend = (arr, name) => {
  const users = arr.filter(el => el.friends.find(el => el === name)).map(el => el.name); // Если какой-либо из элементов массива друзей объекта совпадает с введенным именем, то записать имя этого объекта в новый массив
  return users;
};
